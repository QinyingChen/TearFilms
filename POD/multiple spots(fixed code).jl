...
#=
m2,n2: full domain resolution e.g. 40,40
tspan: e.g. (0,0.5)
t2: number of time steps e.g. 50
x1_shift,y1_shift: shift of the first spot e.g. 1,1
x2_shift,y2_shift: shift of the second spot e.g. -1,-1
sh,sp,sc: number of POD modes for h,p,c e.g. 20,30,20
=#
function PODsol(m2,n2,tspan,t2,x1_shift,y1_shift,x2_shift,y2_shift,sh,sp,sc)

interp_h = zeros(m2*n2,t2)
interp_p = zeros(m2*n2,t2)
interp_c = zeros(m2*n2,t2)
interp_h2 = zeros(m2*n2,t2)
interp_p2 = zeros(m2*n2,t2)
interp_c2 = zeros(m2*n2,t2)
...
sol2 = sol[2]
        ...
        tdata = range(tspan[1],tspan[2],t2)
        ...
        for T in eachindex(tdata)
            ...
            interp_h[:,T] = circle2D(h,x1_shift,y1_shift,m2,n2)
            interp_p[:,T] = circle2D(p,x1_shift,y1_shift,m2,n2)
            interp_c[:,T] = circle2D(c,x1_shift,y1_shift,m2,n2)
           
            interp_h2[:,T] = circle2D(h2,x2_shift,y2_shift,m2,n2)
            interp_p2[:,T] = circle2D(p2,x2_shift,y2_shift,m2,n2)   
            interp_c2[:,T] = circle2D(c2,x2_shift,y2_shift,m2,n2)
    ...
    Dyy = [ q4(mod(i-j,n2)) for i in 1:n2, j in 1:n2 ]
        function PODfun(v, params, t)
          ...
          # Jval = vb1.+(1-vb1)*hump1 .+ vb2.+(1-vb2)*hump2
            ubar = (-h.^2/12).*(Dx*p)
            vbar = (-h.^2/12).*(p*Dy')
            c_x = Dx*c 
            c_y = c*Dy'
        ...
        reducedprob = ODEProblem(POD, newu, tspan,params)
    
        pod_sol = solve(reducedprob, reltol=1e-6, abstol=1e-6)
        return Bh,Bp,Bc,pod_sol
    end

m2 = 40; n2 = 40; t2 = 50;
tspan = (0.0,0.5);
sh = 20; sp = 30; sc = 20;
Bh,Bp,Bc,pod_sol = PODsol(m2,n2,tspan,t2,1,1,-1,-1,20,30,20) 

    theo_data = zeros(3*m2*n2,t2)
    true_data = zeros(3*m2*n2,t2)
